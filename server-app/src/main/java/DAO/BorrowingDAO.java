...
public class BorrowingDAO {
    ...
    public boolean borrowBookTransaction(int userId, int bookId) {
        ...
        String insertBorrowing = "INSERT INTO Borrowings (userId, bookId, borrowDate, status) VALUES (?, ?, GETDATE(), 'borrowing')";
        ...
        try (Connection connection = ConnectDB.getConnection()) {
            ...
            try (
                ...
                boolean success = updateBookStatement.executeUpdate() > 0 && insertBorrowingStatement.executeUpdate() > 0;
                if (success) {
                    connection.commit();
                } else {
                    connection.rollback();
    ...
    public boolean returnBookTransaction(int borrowId, int bookId) {
        String updateBorrowing = "UPDATE Borrowings SET status = 'returned', returnDate = GETDATE() WHERE id = ?";
        String updateBook = "UPDATE Books SET quantity = quantity + 1, loaned_out = CASE WHEN loaned_out > 0 THEN loaned_out - 1 ELSE 0 END WHERE id = ?";
        ...
        try (Connection connection = ConnectDB.getConnection()) {
            ...
            try (
                ...
                boolean success = updateBorrowingStatement.executeUpdate() > 0 && updateBookStatement.executeUpdate() > 0;
                if (success) {
                    connection.commit();
                } else {
                    connection.rollback();
    ...
    public List<String[]> getOverdueBooks(int loanDays, int finePerDay) {
        String sql = "SELECT br.id AS BorrowID, u.username AS BorrowerName, b.title AS BookTitle, " +
